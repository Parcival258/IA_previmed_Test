...
async def responder(mensaje: MensajeEntrada):
    ...
    if intencion == "informacion" and not any(p in texto.lower() for p in ["visita", "médico", "doctor"]):
        completion = cliente_openai.chat.completions.create(
            ...
            messages=[
                {"role": "system",
                 "content": f"Eres el asistente de Previmed. Usa este contexto para responder preguntas generales:\n\n{contexto_prevemed}"},
                {"role": "user", "content": texto},
            ],
        )
    ...
    if intencion == "otro":
        completion = cliente_openai.chat.completions.create(
            ...
            messages=[
                {"role": "system",
                 "content": f"Eres un asistente empático de Previmed. Usa esta información institucional para conversar naturalmente:\n\n{contexto_prevemed}"},
                {"role": "user", "content": texto},
            ],
        )
